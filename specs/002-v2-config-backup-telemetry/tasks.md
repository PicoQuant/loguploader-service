...
- [X] T020a [US1] FR-004a (added 2026-09-08): heartbeat payload gains `instrument_software { version, log_version }` — `version` from the `<install_dir>\<Product>.exe` file-version resource (`version.dll` FFI, `src/identity.rs::control_exe_version`), `log_version` from the newest `*.pqlog` header 3rd `;`-field (`newest_pqlog_version`). Both nullable; reading a log header line is not log collection (FR-001 stands). Verified live: `{version: "1.0.0.5415", log_version: "1.0.0.2094"}` in the backend heartbeat.
